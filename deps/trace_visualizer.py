...
def call_wireshark_for_one_version(wireshark_version, input_file_str, http2ports_string):
    ...
    if len(input_files)>1:
        ...
        mergecap_path = file_name = 'mergecap'
        mergecap_command = [
            mergecap_path,
            '-w',
            output_filename,
    ...
    add_ngap = False
    if platform == 'Linux':
      if version.parse(wireshark_version) >= version.parse('2.9.0'):
          print('Wireshark supports display filter ngap option. Applying')
          add_ngap = True
      else:
          print('Wireshark version <2.9.0. Not applying display filter ngap option')
    else:
      print('\nERROR: Platform {} not supported'.format(platform))
      sys.exit(2)

    display_filter = '(http2 and (http2.type == 0 || http2.type == 1)) or nas-5gs or pfcp'
    if add_ngap:
      display_filter += ' or ngap'
...
if __name__ == '__main__':
    ...
    parser.add_argument('-wireshark', type=str, required=False, default='none', help="If other that 'none' (default), specifies a Wireshark portable version (or list of versions) to be used to decode the input file if that file is a not a PDML file. If more than one version specified, the first one will be used as main version. Other versions will be used as alternatives in case Wireshark reports a malformed packet. In Linux system, only installed version is considered ignoring this parameter")
    ...
    wireshark_version = args.wireshark

    if platform == 'Linux':
       wireshark_version = os.popen("tshark --version | grep ^TShark | awk '{ print $3 }'").read()

    if wireshark_version != 'none':
        input_file = call_wireshark(wireshark_version, input_file, args.http2ports)
    else:
